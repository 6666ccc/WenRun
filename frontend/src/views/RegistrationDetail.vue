  ...
  <AppShell>
    ...
    <UiState :loading="loading" :error="error" :empty="!registration" empty-text="挂号记录不存在">
      <div class="vue-record">
        <section class="card"><div class="flex-between mb-md"><h2>挂号详情</h2><StatusBadge :status="registration.status" :map="REG_STATUS_MAP" /></div>
        ...
        <section v-for="visit in visits" :key="visit.id" class="card"><div class="flex-between mb-md"><h3>就诊记录</h3><StatusBadge :status="visit.status" :map="VISIT_STATUS_MAP" /></div>
