...
function send(text = input.value) { if (!text.trim() || assistant.replying.value || assistant.pendingInterrupt.value) return; assistant.sendMessage(text); input.value = '' }
...
<template>
  <div class="agent-shell">
    ...
    <aside class="agent-history">
      ...
      <p v-if="assistant.sessionError.value" class="agent-history__error">{{ assistant.sessionError.value }}</p>
    ...
    <main class="agent-main">
      ...
      <div v-else class="agent-thread">
        ...
        <article v-for="(message,index) in assistant.activeSession.value.messages" :key="index" class="agent-message" :class="`agent-message--${message.role==='user'?'user':'assistant'}`"><span v-if="message.role!=='user'" class="agent-message__avatar"><UiIcon name="ai" :size="16" /></span><div><p v-if="message.role==='user'">{{ message.content }}</p><template v-else><div v-html="renderMarkdown(message.content)" /><CitationList v-if="message.sources?.length" :sources="message.sources" /></template></div></article>
        ...
        <div v-if="assistant.replying.value && !assistant.streaming.value" class="agent-typing"><span /><span /><span />{{ assistant.streamStatus.value || '正在整理信息…' }}</div><div ref="end" />
      </div>
      <footer class="agent-composer"><div><textarea v-model="input" rows="1" :disabled="assistant.replying.value || !!assistant.pendingInterrupt.value" :placeholder="assistant.pendingInterrupt.value ? '请先确认或拒绝当前挂号请求…' : '描述症状，或告诉我想办理什么…'" @keydown="keydown" /><button v-if="assistant.replying.value" class="agent-composer__stop" @click="assistant.stopReply">停止</button><button v-else :disabled="!input.trim() || !!assistant.pendingInterrupt.value" @click="send()">↑</button></div><p>AI 提供健康信息和就医协助，不替代医生诊断；如遇急症请立即拨打 120。</p></footer>
    </main>
