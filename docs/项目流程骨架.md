...
| Python | 意图路由、RAG、HITL Tool 编排、SSE、知识文档元数据登记 | 不绕过 Java 读取或写入 HIS 业务表 |
...
- **缴费**由患者前端直接打 Java 业务 API。挂号、退号既可走页面，也可由 Agent 在确认后调用 Java 的受限写接口。
- Java 另有处方、发药、检查、药品、库存、仪表盘等接口；当前前端没有对应路由，属于后台/医护能力骨架，未挂到患者门户。
- AI Tool 支持科室、医生、排班、本人挂号查询，以及经 `interrupt` 确认的 `register` / `cancel`；不支持缴费、改排班或访问他人数据。
